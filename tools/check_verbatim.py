#!/usr/bin/env python3
"""СПРАВОЧНЫЙ инструмент: показывает, насколько банки заданий близки к тексту
пособия «Sailing out».

Совпадения с пособием — это НОРМА, а не нарушение. Материал используется с
разрешения авторов и кафедры иностранных языков, а задания на сайте идут
параллельно бумажной книге: чем ближе формулировки, тем удобнее на занятии.
Поэтому находки скрипта ничего не запрещают и переписывать по ним ничего не
нужно — они лишь дают увидеть степень близости к книге (например, когда
хочется понять, какие упражнения переложены свободнее прочих).

В pytest скрипт НЕ подключается и ненулевой код возврата означает просто
«совпадения найдены», а не ошибку.

Совпадением считается цепочка из N слов подряд (по умолчанию 6), встречающаяся
в выгрузке пособия. Речевые клише перечислены в ALLOW и не считаются.
